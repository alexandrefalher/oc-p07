...
def main():
    ...
    combination_result: float = get_combination_result(shares, ShareHolder(), 500)
    print("--------- RESULT ----------")
...
def get_combination_result(shares: ShareHolder, combinations: ShareHolder, limit: float) -> CombinationResult:
    if len(shares) > 0:
        shares_temp: ShareHolder = shares.copy()
        result_1: CombinationResult = get_combination_result(shares_temp, combinations.copy().append(shares_temp.remove(0)), limit)
        result_2: CombinationResult = get_combination_result(shares_temp, combinations, limit)
